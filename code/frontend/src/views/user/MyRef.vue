<template>
  <div style="width:100%; height:100%; background-color:#f5efb3;">
    <div class="rootContainer">
      <div
        class="selectfood"
        style="width:100%; height:100%; background-color:white; display:none; position:fixed; z-index:80;"
      >
        <div style="height:50px; background-color:white" :style="{width:frameSize.x+'px'}">
          <div style="float:right; margin: 11px;" @click="closeGetOneFood">
            <v-icon>mdi-close</v-icon>
          </div>
        </div>
        <get-one-food
          @addfood="addFood"
          :style="{width:frameSize.x+'px', height:(frameSize.y-90)+'px'}"
        ></get-one-food>
      </div>
      <div
        id="dark"
        @click="closeCheckBasket()"
        :style="{width:frameSize.x+'px', height:frameSize.y+'px'}"
        style="background-color:#00000075; z-index:99; position:fixed; display:none;"
      ></div>

      <ref-paging :list-array="foods" @openShare="openShare" id="insideRef"></ref-paging>
      <div id="basket">
        <v-btn
          @click="openCheckBasket()"
          icon
          :style="{width:(frameSize.per*200)+'px',height:(frameSize.per*150)+'px'}"
        >
          <img style="width:100%; height:100%;" src="../../assets/images/basket.png" />
        </v-btn>
      </div>
      <div
        id="FillBtn"
        :style="{'margin-left': (256*frameSize.per)+'px'}"
        style="position:fixed; margin-top: 10px; display:unset;"
      >
      ...
      <div id="shareField" class="inputFeild">
        <!-- 바구니에 넣기 -->
        <div
          style="width: 100%;
    height: 38px;
    ...
    overflow: hidden;"
        >
          {{Nowgra.name_kor}} (재고: {{Nowgra.amount}} )
          <button
            v-on:click="closeShare"
            type="button"
            height="15px"
            width="15px"
            style="float:right;"
          >
            <v-icon size="30px" style="    margin-top: -5px;
">mdi-close</v-icon>
          </button>
        </div>
        <div
          class="textArea"
          style="height: 50px; margin-top: 0px; text-align: center; padding: 5px 36px;;"
        >
          <input
            v-model="totalShareAmount"
            type="text"
            class="inputText"
            style="float: left; width: 40px; height: 35px; text-align: center;"
          />
          <h5 style="font-size:15px">개를</h5>
        ...
        <div class="sharebox" style="display:none;">
          <div class="textArea" style="height:35px; padding:0px;">
            <h5
              class="longNameBox"
              style="width:30%; padding:0px 7px; font-size:15px; font-weight:600;"
            >{{Nowgra.name_kor}}</h5>
            <h5 style="float:left; font-size:15px">와(과) 교환할 재료를 담아주세요</h5>
          </div>
          <div style="width:100%; height:50px; padding: 4px 58px; font-size:15px;">
            <input
              ...
              v-model="nowmyamount"
              class="inputText"
              style="float:left;height:30px;text-align:center;"
            />
            <h5 style="float:left">개당</h5>
            <button
              type="button"
              @click="getFood"
              class="setFood"
            ...
            <input
              ...
              style="float:left; height:30px;text-align:center;"
            />
            <h5 style="float:left">개</h5>
          </div>
          <div style="width:100%; height:40px;">
            <button v-on:click="addChangeGradient" class="tempInputButton">
              <v-icon size="15px">mdi-arrow-down-bold</v-icon>교환목록에 넣기
            </button>
          </div>

          <div
            style="width: 100%; height: 111px;  background-color: rgba(128, 128, 128, 0.2); overflow: scroll; box-shadow: 0px 1px 9px inset #00000042;"
          >
            <div v-for="(food,index) in changeFoodsTemp" :key="index">
              <div class="changeFood" style="font-size:15px;">
                <h4
                  style="float:left; width:90%; float:left;"
                >{{food.Mygradient_kor}} {{food.myamount}}개당 {{food.Cgradient_kor}} {{food.Camount}}개</h4>
                <div style="width:10%; float:left;">
                  <button
                    v-on:click="deleteShareList(index)"
    ...
    margin-top: -7px;"
                  >
                    <v-icon color="white" size="20px">mdi-trash-can-outline</v-icon>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div class="textArea" style="height:130px">
            <textarea
              ...
              placeholder="공유글에 들어갈 글을 적어주세요"
            ></textarea>
          </div>
          <div style="width:100%; height:40px;">
            <button
              v-on:click="putIntoBasket"
              class="tempInputButton"
              style="background-color:rgb(160,212,105);font-size:18px; font-weight:700;"
            >바구니에 넣기</button>
          </div>
        </div>
        <div
          style="width:100%; height:74px; background-color:black; color:white; font-size: 15px; padding: 5px; overflow-y:scroll;"
        >
          ...
          <div
            v-if="trade1api.name!=''"
          >{{trade1api.name}} {{trade1api.unit}} 당 {{trade1api.price}}원</div>
          <div
            v-if="trade2api.name!=''"
          >{{trade2api.name}} {{trade2api.unit}} 당 {{trade2api.price}}원</div>
          <div v-if="myapi.name =='' && trade1api.name==''&&trade2api.name==''">가격 데이터가 존재하지 않습니다.</div>
          <div
            style="color:white; font-size:8px; position:absolute; bottom:0; right:0; "
          >출처 : 농산물 유통정보 KAMIS</div>
        </div>
      </div>
      <!-- end of 바구니에 넣기 -->
      ...
      <div class="registMaterial">
        <!-- 채우기 박스 -->
        <div
          style="width:100%; height:30px; background-color:rgba(224, 224, 224, 0.51); text-align:center; font-weight:bold;text"
        >
          채우기
          <button
            v-on:click="closeregistMater"
            type="button"
            height="15px"
            width="15px"
            style="float:right;"
          >
            <v-icon size="15px">mdi-close</v-icon>
          </button>
        </div>
        <div style="width:100%; height:190px;">
          <div class="textArea">
            <h5>넣을 재료</h5>
            <v-btn @click="getFood" class="fillBtn" style="float:left; width:65px; height:30px;"></v-btn>
          </div>
          <div class="textArea">
            <h5>넣을 개수</h5>
            <input
              ...
              type="text"
              class="inputText"
              style="float:left; width:40px; height:30px;"
            />
            <h5>개</h5>
          </div>
        ...
        <button
          ...
          type="button"
          v-on:click="closeregistMater"
          style="width:100%; height:33px;background-color:rgb(160, 212, 105); font-weight:bold; color:white; font-size:16px;"
        >냉장고에 넣기</button>
      </div>
      <!-- end of 채우기 -->
      ...
      <!-- <div class="checkBasket">
        <div
          style="width:100%; height:30px; background-color:rgba(224, 224, 224, 0.51); text-align:center; font-weight:bold; padding-top:5px; overflow:hidden;"
        >공유 바구니</div>
        <div style="width:100%; height:196px; overflow:scroll; text-align: center;">
          <div
            class="textArea"
            v-for="(food,index) in changeFoods"
            :key="index"
            style="height:auto;"
          >
            <h3>{{food.myfood_kor}}와 교환할 재료</h3>
            <h4 style="fload:left; font-size:12px;">(개당 {{food.price}} 원)</h4>
            <div style=" height: 20px; font-size: 15px;">
              <h4
                style="float:left; width:100%; float:left;"
              >{{food.myfoodcount1}}개당 {{food.tradefood1_kor}} {{food.tradefoodcount1}}개</h4>
            </div>
            <div v-if="food.tradefood2 !=null" style=" height: 20px; font-size: 15px;">
              <h4
                style="float:left; width:100%; float:left;"
              >{{food.myfoodcount2}}개당 {{food.tradefood2_kor}} {{food.tradefoodcount2}}개</h4>
            </div>
          </div>
        </div>
      </div> -->
      ...
      <v-dialog v-model="openBasket" scrollable width= "100%">
        <v-card class="basketlist">
          <v-card-title >
            <div
              style="width:100%; height:30px; text-align:center; font-weight:bold; padding-top:5px; overflow:hidden;"
            >공유 바구니</div>
          </v-card-title>
          <v-divider></v-divider>
          <v-card-text>
            <div
              ...
              style="height:auto;"
            >
              <h3><b style="color: black">{{food.myfood_kor}}</b>와 교환할 재료</h3>
              <h4 v-if="food.price" style="fload:left; font-size:12px;">(개당 {{food.price}} 원)</h4>
              <div style=" height: 20px; font-size: 15px;">
                <h4
                  style="float:left; width:100%; float:left;"
                ><b>{{food.myfoodcount1}}</b>개당 <b style="color:black">{{food.tradefood1_kor}}</b> <b>{{food.tradefoodcount1}}</b>개</h4>
              </div>
              <div v-if="food.tradefood2 !=null" style=" height: 20px; font-size: 15px;">
                <h4
                  style="float:left; width:100%; float:left;"
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="black" text @click="openBasket = false">닫기</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>

      <div
        :style="{'margin-top':frameSize.y-85+'px',width:frameSize.x+'px'}"
        style="position:fixed; bottom: 0;"
      >
        <button
          @click="shareFinish"
          type="button"
          style="width:100%; height:50px;background-color:rgb(160, 212, 105); font-weight:bold; color:white; font-size:20px;"
        >장터에 글올리기</button>
      </div>
    </div>
  </div>
...
const $api_url =
  "https://www.kamis.or.kr/service/price/xml.do?action=dailySalesList&p_cert_key=73081fa5-fa86-492a-b3f3-905e315da6ac&p_cert_id=1137&p_returntype=json";
...
const config = {
  headers: {
    ...
    "Access-Control-Allow-Headers":
      "Origin,Access-Control-Request-Headers, Authorization",
  },
};
...
export default {
  ...
  data() {
    return {
      ...
      openBasket: false, //공유 바구니 오픈/클로즈
  ...
  methods: {
    ...
    deleteFoodfromRef() {
      ...
      axios
        ...
        .post(
          `https://i3b301.p.ssafy.io:9999/food/api/myref/delete`,
          deleteFood
        )
        .then((response) => {
    ...
    fillFoodtoRef() {
      ...
      axios
        ...
        .post(
          `https://i3b301.p.ssafy.io:9999/food/api/myref/regist`,
          registFood
        )
        .then((response) => {
          // console.log(response);
  ...
  created() {
    ...
    axios
      .get(
        `https://i3b301.p.ssafy.io:9999/food/api/myref/search/` +
          this.userinfo.email
      )
      .then((response) => {
